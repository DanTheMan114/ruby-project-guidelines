...
ymddddyooooooooooooooooooooyddddmy       \n ".lines.map { |line| line.center(100) }.join("")
    ...
    def delete_foods
        prompt = TTY::Prompt.new
        choice = prompt.multi_select("\nSelect foods to delete from inventory\n",Food.all.uniq.map{|foo| foo.name})
        choice.select{|ele| Food.where(name: ele).destroy_all}
        puts "These items have been removed from your inventory #{choice}\n"
        sleep(3)
        shopping
    end
    ...
    def create_foods
        ...
        while choice == 'y' or choice == 'Y' do 
            ...
            puts "Enter food category #{Food.all.map{|ele| ele.category}.uniq}"
            food_category = gets.chomp
            puts "Enter food price"
            food_price = gets.chomp
            puts "Enter food hexcode"
            food_hexcode = gets.chomp
            Food.create(name: food_name, category: food_category, price: food_price, hexcode: food_hexcode, tags: 'anonymous',emoji: '😄')
            puts "#{food_name} has been created"
            puts "Would you like to add another food? (y / n)"
            choice = gets.chomp
        end
        shopping
    end
    ...
    def sales
        quantity_by_id = Order.group(:food_id).sum(:quantity)
        total_by_id = Order.group(:food_id).sum(:total)
        table_array = []
        iter = 0
        while iter < quantity_by_id.length do
            table_array << [quantity_by_id.keys[iter],Food.find(quantity_by_id.keys[iter]).name,quantity_by_id.values[iter],total_by_id.values[iter]]
            iter += 1
        end
        puts "Total sales is $#{Order.sum(:total)}"
        table = TTY::Table.new(%w[Food_id Food Quantity Total],table_array)
        colorizer = Lolize::Colorizer.new
        colorizer.write table.render(:ascii, alignments: %i[center]) do |renderer|
        renderer.border.separator = :each_row
        end
    ...
    def shopping
        if @current_user[0].name != "Admin" 
            ...
            prompt = TTY::Prompt.new
            choices = ['🔹Delete Foods'.magenta,'🔹Create Foods'.green, '🔹Sales'.yellow,'🔹Exit'.red]
            system 'clear'
            choice = prompt.select("\n                                                                           🔹Welcome Back!!🔹 \n", choices)
            if choice == '🔹Create Foods'.green
                create_foods
            elsif choice == '🔹Delete Foods'.magenta
                delete_foods
            elsif choice == '🔹Sales'.yellow
                sales
            elsif choice == '🔹Exit'.red
                exit
            end
    ...
    def exit
        @pastel = Pastel.new
        opener
        puts @pastel.cyan(@font.write"                                            Goodbye!    ")
        opener
        switch_song
        play_music('/Users/dantheman/Flatiron/code/ruby-project-guidelines-1/bin/Mario-coin-sound.mp3')
    end
